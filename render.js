import { mat4AsFloatArray } from "./math.js";
import { fetchShader } from "./network.js";
import { mat4, vec3 } from "./vendor/wgpu-matrix/wgpu-matrix.js";
...
export async function initRenderPipeline(args) {
    ...
    const initWallVertexBuffer = (state) => {
        const { walls } = state;
        const vertices = [];
    ...
    function draw(state, computedState) {
    ...
    draw(initialState, initialComputedState);
    {
        ...
        return {
            setState: (newState) => {
                ...
                const updates = typeof newState === "function" ? newState(state, computedState) : newState;
                state = { ...state, ...updates };
